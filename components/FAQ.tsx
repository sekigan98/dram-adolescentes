...
export default function FAQ() {
  return (
    <section
      ...
      className="section-y scroll-mt-32 px-4 sm:px-6 lg:px-8"
    >
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {faqs.map((f, i) => (
            <MotionFade key={f.title} delay={i * 0.1}>
              <article
                className="card p-6 text-center sm:text-left hover:shadow-lg transition-shadow"
                role="group"
                aria-labelledby={`faq-${i}-title`}
              >
                <h3 id={`faq-${i}-title`} className="font-semibold text-brand-800">
                  {f.title}
                </h3>
